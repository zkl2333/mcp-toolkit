...
describe("ExifTool Server Tests", () => {
  ...
  describe("工具注册验证", () => {
    ...
    test("工具应该有正确的描述信息", () => {
      ...
      expect(readMetadataConfig?.description).toContain(
        "读取图片或视频文件的元数据"
      );
      ...
      const extractThumbnailConfig =
        mcpHelper.getToolConfig("extract-thumbnail");
  ...
  describe("MCP 工具调用测试", () => {
    ...
    beforeEach(async () => {
      ...
      testImageFile = await createTestFile(
        tempTestDir,
        "test-image.jpg",
        "fake jpeg content"
      );
    ...
    test("extract-thumbnail 工具 - 应该正确处理输出文件已存在的错误", async () => {
      const outputFile = await createTestFile(
        tempTestDir,
        "existing-thumb.jpg",
        "existing content"
      );
  ...
  describe("敏感操作二次确认", () => {
    test("write-metadata 未确认则取消", async () => {
      ...
      expectToolCall.toContain(result, "操作已取消");
      (server as any).server = original;
    });

    test("delete-metadata 用户拒绝则取消", async () => {
      const original = (server as any).server;
      (server as any).server = {
    ...
    test("extract-thumbnail 确认后不返回取消", async () => {
      const original = (server as any).server;
      (server as any).server = {
        elicitInput: async () => ({
          action: "accept",
      ...
      const input = await createTestFile(
        tempTestDir,
        "thumb-src.jpg",
        "fake content"
      );
      const output = join(tempTestDir, "thumb-out.jpg");
      const result = await mcpHelper.callTool("extract-thumbnail", {
        filePath: input,
        outputPath: output,
        overwrite: true,
      ...
      if (content.includes("操作已取消")) {
        throw new Error(
          "Expected operation to proceed after confirmation, but got cancellation"
        );
      }
      (server as any).server = original;
    });
